// Versión: 1.0.4 – Flujo final probado (tabla visible en /misdatos + respuestas inteligentes)
// Tabla en Supabase: registros_miembros (nombres exactos = glosario)
...
async function iniciarBot() {
  ...
  sock.ev.on('connection.update', async ({ qr, connection, lastDisconnect }) => {
    ...
    if (connection === 'open') { console.log('✅ Bot conectado a WhatsApp'); qrData = null; }
    if (connection === 'close') {
    ...
    for (const m of messages) {
      try {
        ...
        if (pend?.estado === 'solicitar_id' && pend?.who === numero && !lower.startsWith('/')) {
          ...
          if (!savedFP) {
            await supabase.from(TABLE).update({ whatsapp_id: currentFP }).eq("id", found.id);
          }
        ...
        if (pendCel?.estado === 'solicitar_cel' && pendCel?.who === numero && !lower.startsWith('/')) {
          ...
          const { data: reg, error } = await supabase.from(TABLE).select('*').eq('id', pendCel.id).maybeSingle();
          if (error || !reg) { await enviar(sock, from, '⚠️ Error al verificar. Intenta de nuevo.'); borrar(PENDIENTE_FILE); continue; }
...
if (lower.startsWith('/actualizacion')) {
  ...
  continue;
        ...
        if (rest?.estado === 'pedir_id' && rest?.who === numero && !lower.startsWith('/')) {
          ...
          if (errR || !foundR) { await enviar(sock, from, '❌ No encontré registro.'); borrar(RESTAURAR_FILE); continue; }
