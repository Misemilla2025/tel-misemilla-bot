...
// === Reintento programado cada 15 minutos (mantener conexión) ===
setInterval(() => {
  iniciarBot().catch(err => console.error('⏳ Reinicio automático forzado:', err));
}, 1000 * 60 * 15);

// === Auto-ping interno cada 4 minutos (mantener Render activo) ===
setInterval(() => {
...
// === Servidor Express para mantener el contenedor despierto ===
...
// === Monitor de conexión Baileys (reconexión automática) ===
setInterval(() => {
  try {
    if (!global.sock || !global.sock.ws || global.sock.ws.readyState !== 1) {
      console.log('🔄 Socket inactivo, intentando reconectar...');
...
// === Verificación periódica del archivo de sesión ===
import fs from 'fs';
import path from 'path';
const AUTH_PATH = path.join('./auth_info_full');

setInterval(() => {
  try {
    if (fs.existsSync(AUTH_PATH)) {
      console.log('💾 Sesión auth verificada.');
    } else {
      console.warn('⚠️ No se encontró la sesión, intentando restaurar...');
      iniciarBot().catch(err => console.error('🔁 Reintento tras pérdida de auth:', err));
    }
  } catch (err) {
    console.error('⚠️ Error en respaldo de sesión:', err);
  }
}, 1000 * 60 * 10);

// === Evita que Render cierre el proceso por errores no controlados ===
