...
// === Servidor Express + Ruta /health para UptimeRobot ===
const express = require('express');
...
// Endpoint de salud (Render/UptimeRobot)
...
// === Guardado de sesión en Supabase (ajuste anti-duplicado) ===
async function guardarSesionSupabase(payload) {
  try {
    const { error } = await supabase
      .from('sesion_whatsapp')
      .upsert(
        { nombre: 'mi_sesion', datos: payload, updated_at: new Date().toISOString() },
