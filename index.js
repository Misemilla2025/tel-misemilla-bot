...
if (rest?.estado === 'pedir_id' && rest?.who === numero && !lower.startsWith('/')) {
  ...
  if (errR || !foundR) {
    await enviar(sock, from, '❌ No encontré registro asociado.');
...
if (rest?.estado === 'pedir_nuevo_cel' && rest?.who === numero && !lower.startsWith('/')) {
  ...
  await enviar(sock, from, `⚠️ Estás a punto de restaurar tu cuenta con el número: *${nuevoCel}*\n¿Confirmas que deseas continuar?\nResponde con *sí* o *no*.`);
  continue;
}
...
if (rest?.estado === 'confirmar' && rest?.who === numero && !lower.startsWith('/')) {
  if (['si','sí','s'].includes(lower)) {
    try {
      const { error: errU } = await supabase.from(TABLE).update({ celular: rest.nuevo }).eq('id', rest.id);
      if (errU) {
        await enviar(sock, from, '❌ No fue posible completar la restauración.');
        console.error('Error actualización:', errU);
      } else {
        await enviar(sock, from, '✅ *Restauración completada.*\nTu número fue actualizado correctamente.\nYa puedes usar */misdatos* y */actualizacion*.');
      }
    } catch (e) {
      await enviar(sock, from, '⚠️ Error inesperado en la restauración.');
      console.error(e);
    }
  } else if (['no','n'].includes(lower)) {
    await enviar(sock, from, '❌ Restauración cancelada por el usuario.');
  } else {
