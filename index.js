...
async function saveSessionToSupabase() {
  try {
    ...
    if (error) {
      console.error('⚠️ Supabase saveSession error:', error);
    } else {
      console.log('💾 Sesión guardada o actualizada en Supabase (sesion_whatsapp).');
    }
  } catch (e) {
...
async function loadSessionFromSupabase() {
  try {
    ...
    if (error) {
      console.error('⚠️ Supabase loadSession error:', error);
      return;
    }

    if (!data || !data.datos) {
      console.log('🕒 No se encontró sesión previa en Supabase.');
      return;
    }
...
iniciarBot()
  ...
  .catch(e => console.error('❌ Error al iniciar el bot:', e));

// === 🔁 Reintento programado cada 15 minutos (mantener conexión activa) ===
setInterval(() => {
  console.log('⏳ Reintentando mantener sesión activa...');
  iniciarBot().catch(err => console.error('⚠️ Reinicio automático forzado:', err));
}, 1000 * 60 * 15); // cada 15 min

// === 🌐 Servidor Express + Ruta /health (para Render y UptimeRobot) ===
const express = require('express');
...
// === ❤️ Auto-ping interno cada 4 minutos (mantener Render activo) ===
setInterval(() => {
...
// === 💾 Guardado seguro de sesión en Supabase (sin duplicados) ===
  ...
  try {
    const { error } = await supabase
      ...
      .upsert(
        ...
        { onConflict: 'nombre' } // evita duplicados
      );

    if (error) {
      console.error('⚠️ Error guardando sesión en Supabase:', error.message);
    } else {
      console.log('✅ Sesión guardada o actualizada correctamente en Supabase.');
    }
  } catch (e) {
...
// === 🧠 Monitor automático de errores Baileys + limpieza de sesión ===
process.on('uncaughtException', err => {
  const msg = err.message || '';

  if (msg.includes('stream errored') || msg.includes('conflict') || msg.includes('replaced')) {
    console.warn('⚠️ Conexión de WhatsApp interrumpida por conflicto o reemplazo.');

    try {
      const AUTH_PATH = path.join('./auth_info_multi');
      if (fs.existsSync(AUTH_PATH)) {
        fs.rmSync(AUTH_PATH, { recursive: true, force: true });
        console.log('🧹 Carpeta de autenticación eliminada para forzar nueva sesión limpia.');
      }
    } catch (cleanErr) {
      console.error('⚠️ Error al limpiar sesión:', cleanErr.message);
    }

    console.log('🔁 Reiniciando bot en 8 segundos...');
    setTimeout(() => {
      iniciarBot().catch(e => console.error('❌ Error al reiniciar tras limpieza:', e));
    }, 8000);
  } else {
...
process.on('unhandledRejection', reason => {
  console.warn('⚠️ Rechazo no controlado detectado:', reason);
});
