// === MI SEMILLA BOT – Versión Estable ===
// Autor: Ale (Total Business)
// Fecha: Octubre 2025
// Descripción: Bot de WhatsApp con conexión Supabase + Render
// =============================================================
...
async function iniciarBot() {
  ...
  sock.ev.on("creds.update", async () => {
    try {
      ...
      console.error("⚠️ Error al actualizar credenciales:", err.message);
    }
  });
  ...
  sock.ev.on("connection.update", ({ connection, lastDisconnect, qr }) => {
    if (qr) {
      console.log("🔗 Escanea el QR desde /qr o la consola para conectar tu sesión.");
    ...
    if (connection === "open") {
      ...
      if (code !== DisconnectReason.loggedOut) {
        console.log("🔁 Reintentando conexión en 4 segundos...");
        setTimeout(iniciarBot, 4000);
      } else {
        console.log("🚫 Sesión cerrada permanentemente. Elimina auth_info_full para reconectar.");
      }
    }
  });
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    for (const m of messages) {
      try {
        ...
        console.log("JID:", from, "→ Número:", numero);

        // (Aquí continuarán tus comandos personalizados)
      } catch (err) {
...
const app = express();
app.get("/", (req, res) => {
  res.send("🌱 Bot Mi Semilla – WhatsApp conectado correctamente");
});
